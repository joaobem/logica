...
def aumento_salarial(salario, porcentagem):
    ...
    aumento = aumento_salarial + salario 
...
def dois_elevado_a_um_milhao():
    ...
    algarismos"""
    dois_elevado_a_um_milhao = 2**1000000
    result = str(dois_elevado_a_um_milhao)
    return len(result)
...
def salario(valor_hora, horas_mensais):
    ...
    quase = valor_hora * horas_mensais
    muitoquase = (quase * 24)/100
    salario = quase - muitoquase
    return salario
...
def tinta(metros_pintar):
    ...
    tinta = math.ceil(metros_pintar/3/18)
    return tinta
...
def imprime_mes_por_extenso(data):
    """Faça um programa que solicite a data de nascimento (dd/mm/aaaa)unidade = numero % 10
...
def é_azarado(numero):
    """O último dígito não pode ser igual ao primeiro, porque isso dá unidade = numero % 10ar."""


def ondernamento_contrario(lista):
    """ Devolve a lista invertida"""
    ondernamento_contrario = list(reversed(lista))
...
def media_saltos_lista(saltos):
    ...
    media_saltos_lista = saltoss/media_saltos
    return round(media_saltos_lista,1)

def contem(lista, item_procurado):
    """Verifica se uma lista contém um item e devolve um valor booleano."""
    contem = item_procurado in (lista)
    return contem
...
def media_temperaturas(temperaturas):
    ...
    """
    temps = sum(temperaturas)
    tempo = len(temperaturas)
    media_temperaturas = temps/tempo
    return round(media_temperaturas)
